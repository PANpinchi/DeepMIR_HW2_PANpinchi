...
class OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        if not remove_lstm:
            ...
            self.lstm = Linear(hidden_size, hidden_size, bias=False)
...
class Separator(nn.Module):
    ...
    def __init__(
        ...
        self.use_griffinlim = use_griffinlim
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        if self.use_griffinlim:
            if self.residual:
                # Calculate residual by subtracting the sum of all other targets from the mixture
                sum_estimates = torch.sum(spectrograms, dim=-1, keepdim=True)
                residual_spectrogram = X - sum_estimates.squeeze(-1)  # X is the original mixture spectrogram
                spectrograms = torch.cat((spectrograms, residual_spectrogram.unsqueeze(-1)), dim=-1)
                nb_sources += 1
            ...
            nb_channels = spectrograms.shape[-2]
            nb_sources = spectrograms.shape[-1]
            estimates_list = []
                ...
                for channel in range(nb_channels):
                    ...
                    for source in range(nb_sources):
                        ...
                        while pos < nb_frames:
                            ...
                            tmp = spectrograms[sample, cur_frame, :, channel, source].cpu().numpy().T

                            estimated_waveform = librosa.griffinlim(tmp)

                            estimates_source.append(torch.from_numpy(estimated_waveform).to(audio.device))
                        estimates_channel.append(torch.cat(estimates_source, dim=0))
                    estimates_sample.append(torch.stack(estimates_channel, dim=0))
                estimates_list.append(torch.stack(estimates_sample, dim=0))
            estimates = torch.stack(estimates_list, dim=0)
        else:
            # transposing it as
            # (nb_samples, nb_frames, nb_bins,{1,nb_channels}, nb_sources)
            spectrograms = spectrograms.permute(0, 3, 2, 1, 4)
