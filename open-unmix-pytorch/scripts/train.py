...
from my_openunmix import data, model, utils, transforms
...
def get_statistics(args, encoder, dataset, device):
    ...
    for ind in pbar:
        ...
        scaler.partial_fit(np.squeeze(X.cpu()))
...
def main():
    ...
    parser.add_argument(
        "--is_wav", action="store_true", default=False, help="is_wav"
    )
    parser.add_argument(
        "--remove_lstm", action="store_true", default=False, help="remove_lstm"
    )
    ...
    val_dataloader_kwargs = {"num_workers": 0, "pin_memory": True} if use_cuda else {}
    ...
    # args.source_augmentations = ['gain', 'channelswap', 'force_stereo']
    ...
    valid_sampler = torch.utils.data.DataLoader(valid_dataset, batch_size=1, **val_dataloader_kwargs)
    ...
    if args.checkpoint or args.model or args.debug:
        ...
        scaler_mean, scaler_std = get_statistics(args, encoder, train_dataset, device)
    ...
    # Store SDR results for specific epochs
    sdr_results = {}
    ...
    for epoch in t:
        ...
        # Save model for specific epochs: 25, 50, and 150
        if epoch in [25, 50, 150]:
            utils.save_checkpoint(
                ...
                is_best=True,
                path=target_path,
                target=args.target + "_" + str(epoch),
            )
            print("Model saved for epoch {} at {}".format(epoch, os.path.join(target_path, args.target + "_" + str(epoch) + ".pth")))
    ...
    # Save SDR results
    with open(Path(target_path, "sdr_results.json"), "w") as outfile:
        json.dump(sdr_results, outfile, indent=4)
